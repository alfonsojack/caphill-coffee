describe('caphill coffee shops', () => {
  ...
  it('shows coffee shop cards on load, and can navigate shop', () => {
    ...
    cy.get('[href="/SelectedShop/1"] > .card > .card-rating').contains("h4", "Rating: 100%");
    cy.get(".card-container").last().should("contain", "Pablos Coffee");
    cy.get('img[alt="Pablos Coffee image"]').should('have.attr', 'src', "https://lh3.googleusercontent.com/p/AF1QipPb-7--lSkbxVIzioimmGN_bU2KZZrq5EpBw2-f=s1360-w1360-h1020"); //change to real path
    cy.get('[href="/SelectedShop/3"] > .card > .card-rating').contains("h4", "Rating: 25%");
    cy.get('[href="/SelectedShop/3"] > .card').click();
    cy.url().should("include", 'SelectedShop/3')
    cy.get(".shop-name").should('contain', "Pablos Coffee")
    // nicole's tests for content of details
